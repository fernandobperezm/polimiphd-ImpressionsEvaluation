...
class MINDSmallRawData:
    ...
    def _download_dataset(self) -> None:
        ...
        if (
            not os.path.exists(self._original_dataset_train_file)
            and not os.path.exists(self._original_dataset_validation_file)
        ):
            logger.info(
            ...
            train_small_filename = 'MINDsmall_train.zip'
            validation_small_filename = 'MINDsmall_dev.zip'

            local_train_zip_path = download_remote_file(
                url=f"{base_url}/{train_small_filename}",
                destination_filename=os.path.join(
                    self._original_dataset_root_folder, train_small_filename,
                )
            )

            local_validation_zip_path = download_remote_file(
                url=f"{base_url}/{validation_small_filename}",
                destination_filename=os.path.join(
                    self._original_dataset_root_folder, validation_small_filename,
                )
            )

            logger.info(
                f"Extracting {local_train_zip_path} into {self._original_dataset_train_folder}"
            )
            ...
            logger.info(
                f"Extracting {local_validation_zip_path} into {self._original_dataset_validation_folder}"
            )
            with zipfile.ZipFile(local_validation_zip_path, 'r') as zip_ref:
                zip_ref.extractall(
                    self._original_dataset_validation_folder
                )

            logger.info(
                f"Deleting temporal ZIP files: {local_train_zip_path} and {local_validation_zip_path}"
            )
            os.remove(local_train_zip_path)
            os.remove(local_validation_zip_path)


class MINDSmallDataFrames(Enum):
...
class PandasMINDSmallRawData:
    """A class that reads the MINDSmall data using Pandas Dataframes."""

    def __init__(
        self,
    ):
        self.config = MINDSmallConfig()
        self.raw_data_loader = MINDSmallRawData()
    ...
    def _df_train_and_validation(self) -> pd.DataFrame:
        ...
        df_validation["impression_id"] += self.raw_data_loader.num_train_data_points
...
class MINDSmallReader(BaseDataReader):
    DATA_FOLDER = os.path.join(
        ".",
        "data",
        "MIND-SMALL",
    ...
    _DATA_READER_NAME = "MINDSmallReader"

    DATASET_SUBFOLDER = "MIND-SMALL/"
...
if __name__ == "__main__":
    raw_data = MINDSmallRawData()

    pandas_data = PandasMINDSmallRawData()

    all_data_reader = MINDSmallReader()
